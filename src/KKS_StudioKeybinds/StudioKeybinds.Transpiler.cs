    ...
    class MainShortcutPatch
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            for (int i = 0; i < code.Count -1; i++)
            {
                ...
                MethodInfo gameExitInfo = AccessTools.Method(typeof(ExitDialog), nameof(ExitDialog.GameEnd), new Type[] { typeof(bool) });
                if (code[i].opcode == OpCodes.Ldc_I4_0 && code[i + 1].opcode == OpCodes.Call && (MethodInfo)code[i + 1].operand == gameExitInfo)
                {
